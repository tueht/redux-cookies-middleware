        ...
        Object.keys(paths).forEach((pathToState) => {
            ...
            if (!equalityCheck(prevVal, nextVal)) {
                if (deleteCheck(nextVal)) {
                    options.setCookie(state.name, JSON.stringify(nextVal), 0, state.secure, state.sameSite);
                } else {
                    options.setCookie(state.name, JSON.stringify(nextVal), state.expiry, state.secure, state.sameSite);
                }
            }
        });
